import json,sqlite3,requests,discord,random,time
...
#АКТИВИТИ#
@bot.command()
async def ytt(ctx):# YouTube Together
...
@bot.command()
async def fish(ctx):# Fishington.io
    data = {
        "max_age": 86400,
        "max_users": 0,
    ...
    await ctx.send(f"https://discord.com/invite/{link['code']}")
@bot.command()
async def poker(ctx):# Poker Night
    data = {
        "max_age": 86400,
        "max_users": 0,
...
async def on_ready():
    ...
    connection.commit()
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name='сломай нервы разраба'))
    print('bot connected')
...
@bot.command(aliases = ['добавить магазин'])
...
@bot.command(aliases = ['удалитьмагазин'])
...
@bot.command(aliases = ['лидеры', 'лб', 'lb'])
...
async def info(ctx, member: discord.Member = None):
    ...
    if member is None:
        ...
        emb.set_footer(text="C любовью MAKIGO", icon_url=bot.user.avatar_url)
...
async def on_message(message):
    if ">balance" in message.content:
          await message.channel.send("test-bot pidr")
